 ...
 */
import { RAW_GAME_DATA } from './game-data.js';
import { GameDataSchema, type GameData } from './schema.js';

function parseGameData(): GameData {
  const result = GameDataSchema.safeParse(RAW_GAME_DATA);
  if (!result.success) {
    const issues = result.error.issues
      ...
      .join('\n');
    throw new Error(
      `game-data không đúng hình dạng mong đợi.\n${issues}\n\n` +
