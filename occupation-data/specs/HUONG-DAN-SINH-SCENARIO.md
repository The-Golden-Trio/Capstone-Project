...
| **scenario_title** | Tên kịch bản, **chính là chuỗi task** chép nguyên văn |
| **activity** | Nhịp viết sẵn trong file. Mỗi kịch bản có 3–4 cái, nối nhau bằng `forward_to`. Mỗi activity có khối `context` riêng |
| **follow-up** | Câu đào sâu, **runtime AI tự sinh**, không viết sẵn. File chỉ khai trần `limitFollowup` và `followup_goal` |
| **hint** | Gợi ý người chơi chủ động bấm xem. Dùng hint thì activity đó tối đa mốc `0` |
| **quick action** | Activity gấp có đồng hồ đếm ngược cưỡng chế. Hết giờ tính `-1` |
| **observes** | Rubric. Mỗi activity khai nó quan sát kỹ năng nào, kèm 3 mốc hành vi `+2` / `0` / `-1` |
...
**Ba luật quan trọng nhất, nhớ mỗi ba cái này thôi cũng được:**

1. **Một task một kịch bản.** Level có 3 task thì có đúng 3 kịch bản, tên kịch bản là chuỗi task.
2. Cùng một nghề ở band khác nhau **phải chơi khác nhau**. Kịch bản dán được sang band khác mà không phải sửa gì thì nó sai.
3. `FREETEXT` là **mặc định**. `CHOICE` tối đa 1 mỗi kịch bản và phải giải thích vì sao bắt buộc.
...
[1] Nguyên KHỐI A của spec-scenario-KHOI1.md
    ...
    — tới hết mục "A15. Mười chín lỗi hay gặp"
    — KHÔNG dán KHỐI B
...
**Số scenario = số task của level.** Nhìn `level.tasks` ở bước 2 có mấy phần tử thì sinh bấy nhiêu kịch bản. Thường là 3.

Mỗi kịch bản chọn một archetype **theo nội dung task**, và archetype đó phải nằm trong nhóm đã mở ở band. Trần follow-up (A7) đổi theo band:

| Band | Archetype được phép chọn | Trần follow-up mỗi kịch bản |
|---|---|---|
| L1 | `S_EXEC` | ≤ 2 |
| L2 | thêm `S_AMBIG` | ≤ 2 |
| L3 | thêm `S_INCIDENT` | ≤ 3 |
| L4–L5 | thêm `S_CONFLICT`, `S_REVIEW` | ≤ 3 |
| L6+ | thêm `S_DECISION` | ≤ 4 |

Ví dụ `SWE_BACKEND` L3 có 3 task → 3 kịch bản, lần lượt hợp với `S_EXEC`, `S_AMBIG`, `S_INCIDENT`.

Ở band cao sẽ **có archetype không dùng tới** — L6 mở 6 archetype nhưng chỉ 3 task. Đó là kết quả đúng, đừng nhồi thêm kịch bản cho đủ.

Nên sinh cả bộ trong một lượt để AI tự tránh trùng cảnh giữa chúng.

---

## 4. Sinh thêm cho job đã có scenario

Từ `2.1`, trùng **task** đã là lỗi chính thức (A3) và script kiểm bắt được. Nhưng trùng **cảnh** thì máy không bắt được — hai kịch bản khác task vẫn có thể cùng dựng cảnh An ngồi cạnh lúc 9 giờ sáng. Khi sinh lẻ, chặn bằng cách dưới.
...
Sinh kịch bản cho hai task còn lại. Mỗi cái neo vào một task KHÁC
...
print(ok(3 <= len(acts) <= 4), f"so activity co dinh = {len(acts)} (can 3-4)")

# --- A7: CHOICE la ngoai le ---
ch = [b for b in acts if b['type']=='CHOICE']
...
print(ok(all((b['options'] is None)!=(b['input_prompt'] is None) for b in acts)), "options/input_prompt loai tru")

# --- A7: do thi activity ---
ids = {b['activity_id'] for b in acts}
origins = [b for b in acts if b.get('isOrigin')]
reach = set(); cur = origins[0]['activity_id'] if origins else None
while cur and cur != 'END' and cur not in reach:
    reach.add(cur); cur = next(b['forward_to'] for b in acts if b['activity_id']==cur)
print(ok(len(origins)==1), f"dung 1 isOrigin (co {len(origins)})")
print(ok(all(b['forward_to'] in ids or b['forward_to']=='END' for b in acts)), "forward_to tro toi activity co that")
print(ok(sum(1 for b in acts if b['forward_to']=='END')==1), "dung 1 activity forward_to=END")
print(ok(reach==ids), "moi activity toi duoc tu goc", (ids-reach) or "")

# --- A7: tran follow-up + ngan sach ---
cap = {1:2,2:2,3:3,4:3,5:3}.get(bn, 4)
tot = sum(b['limitFollowup'] for b in acts)
print(ok(tot<=cap), f"tong limitFollowup = {tot} <= tran band {cap}")
print(ok(all(b['limitFollowup']<=2 for b in acts)), "moi activity <= 2 followup")
print(ok(all(b['limitFollowup']==0 for b in acts if b['type']=='CHOICE' or b.get('quick_action'))),
      "CHOICE/quick_action co limitFollowup=0")
print(ok(all((b['followup_goal'] is None)==(b['limitFollowup']==0) for b in acts)), "followup_goal khop limitFollowup")
budget = 2*sum(1 for b in acts if b['type']=='FREETEXT') + 1*len(ch) + 1.5*tot + 2
print(ok(10<=budget<=15), f"ngan sach = {budget} phut | khai bao {ctx['estimated_minutes']}")

# --- A9 / A10 / A11 ---
print(ok(all(len(b['hints'])<=1 for b in acts)), "hints toi da 1")
print(ok(all(not b['hints'] for b in acts if b['type']=='CHOICE' or b.get('quick_action'))),
      "CHOICE/quick_action khong co hint")
print(ok(all(b.get('closing_prompt') for b in acts)), "moi activity co closing_prompt")
qa = [b for b in acts if b.get('quick_action')]
...
print(ok(all(r['after_activity'] in ids for r in re_)), "after_activity co that")
...
if lv:
    ...
    print(ok(soft <= {o['skill'] for b in acts for o in b['observes'] if o['skill_type']=='soft'}), "phu het skills_soft")
else:
    print("--   task/skill nguyen van: BO QUA (tier 2/3, phai doc bang mat)")
bad = [(b['activity_id'], o['skill']) for b in acts for o in b['observes']
...
# --- 2.1: title, skills_in_play, job_scope ---
print(ok(s['scenario_title']==ctx['task']), "scenario_title == context.task")
mismatch = [a['activity_id'] for a in acts
            if set(a['context']['skills_in_play']) != {o['skill'] for o in a['observes']}]
print(ok(not mismatch), "skills_in_play khop observes", mismatch or "")
print(ok(all(a['context'].get('job_scope') for a in acts)), "moi activity co job_scope")
...
tot_obs = sum(len(b['observes']) for b in acts)
...
dup = [o['context']['scenario_archetype'] for o in same if o['context']['task']==ctx['task']]
print(ok(not dup), "khong trung task voi scenario khac cung job", dup or "")
...
2. **Ba phương án của activity `CHOICE`, có cái nào ngu ngốc hiển nhiên không?** Nếu bạn loại được một phương án trong nửa giây thì activity đó chỉ còn hai lựa chọn.
3. **`choice_reason` có thuyết phục không?** Nếu tình huống đó vẫn gõ tự do được thì nó phải là `FREETEXT`. Mỗi activity `CHOICE` là một activity đánh mất cơ hội quan sát.
4. **Mốc hành vi có viết bằng tính từ không?** "Giao tiếp tốt" là sai. Phải là hành vi quan sát được.
5. **Hint có đưa thẳng đáp án không?** "Em xem log DB chưa?" là đúng. "Đây là N+1 query" là sai — nó cho luôn thứ đang được đo, và activity mất giá trị kể cả khi đã hạ trần.
...
9. **Ending `SECRET` (nếu có): có activity nào cho phép người chơi làm điều đó không?** Không có thì đó là kết cục chết.
...
| Ngân sách vượt 15 phút | Quá nhiều activity `FREETEXT` | Bớt activity, **không** rút ngắn nội dung activity để lách |
...
| Sinh 2–3 activity `CHOICE` | AI vẫn theo thói quen cũ, `CHOICE` dễ viết hơn | Nhắc A7: `FREETEXT` là mặc định, `CHOICE` tối đa 1 và phải có `choice_reason` |
...
| Activity không ai trỏ tới | `forward_to` gãy | Rà lại chuỗi `isOrigin` → ... → `END`, sửa đúng con trỏ bị sai |
